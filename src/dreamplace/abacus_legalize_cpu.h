  ...
  for (int i = 0; i < num_nodes; i += 1)
  {
    ...
    else
    {
      ...
      int bin_id_xh = std::min(
          (int)ceilDiv(x[node_id] + node_size_x[node_id] - xl, bin_size_x, NumericTolerance<T>::rtol * site_width / bin_size_x), num_bins_x);
                       ...
                       const int num_row_nodes)
{
  ...
  auto collapse = [&](int cluster_id, T range_xl, T range_xh)
  {
    ...
    while (true)
    {
      ...
      dreamplaceAssertMsg(cluster->x + site_width * NumericTolerance<T>::rtol >= range_xl && cluster->x + cluster->w <= range_xh + site_width * NumericTolerance<T>::rtol,
                          "%.6f >= %.6f && %.6f + %.6f <= %.6f",

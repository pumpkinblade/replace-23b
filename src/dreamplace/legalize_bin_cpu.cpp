        ...
        ) 
{
    for (int i = 0; i < num_bins_x*num_bins_y; i += 1) 
    {
        ...
        for (int ci = bin_cells.at(i).size()-1; ci >= 0; --ci)
        {
            ...
            std::vector<int> blank_index_offset(num_node_rows, 0);
            ...
            std::vector<int> best_blank_bi(num_node_rows, -1);
            ...
            for (int bin_id_offset_y = 0; abs(bin_id_offset_y) < blank_bin_id_dist_y; bin_id_offset_y = (bin_id_offset_y > 0)? -bin_id_offset_y : -(bin_id_offset_y-1))
            {
                ...
                std::vector<int> row_best_blank_bi(num_node_rows, -1);
                ...
                for (unsigned int bi = 0; search_flag && bi < bin_blanks.at(blank_bin_id).size(); ++bi)
                {
                    ...
                    std::fill(blank_index_offset.begin() + 1, blank_index_offset.end(), -1);
                    ...
                    while (true)
                    {
                        ...
                        if (intersect_blank_width >= width)
                        {
                            ...
                            if (cost < row_best_cost)
                            {
                                ...
                                std::copy(blank_index_offset.begin(), blank_index_offset.end(), row_best_blank_bi.begin());
                                row_best_cost = cost; 
                ...
                if (row_best_cost < best_cost)
                {
                    ...
                    std::copy(row_best_blank_bi.begin(), row_best_blank_bi.end(), best_blank_bi.begin());
